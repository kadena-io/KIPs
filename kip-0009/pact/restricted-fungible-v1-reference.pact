...
(module restricted-fungible-v1-reference GOVERNANCE

  (implements fungible-v2)
  (implements restricted-fungible-v1)
  (use fungible-util)
  ...
  (defcap TRANSFER:bool
    ...
    (let ((r (detect-transfer-restriction sender receiver amount)))
      (enforce (= r "") (format "Transfer restriction detected: {}" [r])))
  ...
  (defun detect-transfer-restriction:string
      ( sender:string
        receiver:string
        amount:decimal )
    @doc "Hard-coded test implementation"
    (if (= sender "Bob")
      (if (= receiver "Alice")
        (if (> amount 10.0) "Transfer restriction Bob->Alice > 10.0"
          "")
        "")
      "")
  )
