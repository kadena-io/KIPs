...
1. `requiredNamespaces`: `Object` 
   1.1. `chains`: `Array<string>` - an array of [Chain IDs](#wallet-connect-chain-ids)
...
```
    <namespace> + ":" + <reference>
```
- `<namespace>` always refers to "kadena".
- `<reference>` refers to a unique network ID within the blockchain ecosystem (e.g. testnet, mainnet).
...
> WalletConnect and is not equivalent to Kadena's "chain IDs".
...
- For all other cases, the [`kadena_get_accounts_v1`](#kadena_get_accounts_v1)
