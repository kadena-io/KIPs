...
Sign is part of the [Kadena Signing API](https://github.com/kadena-io/signing-api) and is most commonly used by the [Chainweaver](https://github.com/kadena-io/chainweaver/releases) wallet. This method allows the wallet to show the user a **single** transaction that needs signature approval.
...
// kadena_sign_v1 Method Request
{
  ...
  "params": {
    SigningCmdRequest // Chainweaver type
  }
}
```
...
// kadena_sign_v1 Response
{
    ...
    "jsonrpc": "2.0",
    "result":  {
      SigningResponse // Chainweaver type
    }
}
```
...
1. `params`: `Object` - A [`SigningCmdRequest`](https://kadena-io.github.io/signing-api/#/definitions/SigningRequest) (a Chainweaver type) containing information the wallet needs to create and sign the transaction.

#### kadena_sign_v1 Response

The method response expects a `result` object with the following properties:

1. `result`: `Object` - A [`SigningResponse`](https://kadena-io.github.io/signing-api/#/definitions/SigningResponse) (a Chainweaver type) that contains a signed command ready to be submitted to the blockchain.
