...
```
{
  "cmds": [ { "cmd": string, "sigs": [ { "pubKey":string, "sig": string|null }]}]
}
```
...
- The `cmds` field contains the list of requests; some of which contain new signatures attached. Even in the case of an error, it is still useful to have the ordered list of `CommandSigDatas` in the response, given that some of them will contain new signatures.
...
For when the `cmds` field in the request is an empty list
