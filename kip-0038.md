...
accounts. When an account isn't minted, the `chainAccounts` field will be an
empty array.
...
2. **Error Handling:**
   - **MUST** return a clear and descriptive error message if account
     information cannot be retrieved.
   - Errors may result from configuration issues, unsupported networks, or other
     internal errors.
...
- two accounts with from different `networkIds`: `mainnet01` and `testnet04`
- two accounts on different contracts: `coin` and `kdx`
...
### Example Response: Single Account, Multiple Guards

This account with account name `test` has two guards.

- The account that is valid for the first guard is on chains `0`, `2`, `3`, and
  `6`.
- The account that is valid for the second guard is on chains `1`.
...
### Example response: Non-minted account
